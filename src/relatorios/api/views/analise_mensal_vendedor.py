...
class AnaliseMensalVendedorView(APIView, DataFrameGenerics):
    """Relatório de análise mensal de vendas por vendedor para consumo no front"""
    ...
    def _vendas(self)-> pd.DataFrame:
        ...
        qs = (
            ...
            .values('mes', 'valor','vendedor__vendedor', 'vendedor__foto'))
        return (
            pd.DataFrame(qs)
            .rename(columns= {
                'mes':'data',
                'vendedor__vendedor': 'vendedor',
                'vendedor__foto': 'foto'})
            .pipe(self.convert_to_float, 'valor')
            .groupby(['data','vendedor','foto'])
            .agg({'valor': 'sum'})
            .reset_index()
