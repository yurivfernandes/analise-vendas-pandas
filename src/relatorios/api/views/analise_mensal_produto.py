...
class AnaliseMensalProdutoCodigoView(APIView, DataFrameGenerics):
    """Relatório de análise mensal de vendas por codigo para consumo no front"""
    ...
    def _vendas(self)-> pd.DataFrame:
        ...
        qs = (
            ...
            .values('mes', 'valor','produto__codigo', 'produto__linha', 'produto__grupo'))
        return (
            pd.DataFrame(qs)
            .rename(columns= {
                'mes':'data',
                'produto__codigo': 'codigo',
                'produto__linha': 'linha',
                'produto__grupo': 'grupo'})
            .pipe(self.convert_to_float, 'valor')
            .groupby(['data','codigo','linha','grupo'])
            .agg({'valor': 'sum'})
            .reset_index()
